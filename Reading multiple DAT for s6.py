...
os.chdir('C:/Users/SCRAJU88/Documents/Main Docs/Life Stuff/Product Ideas/Pediabyte/Smartphone Reflectometry/Data/Test Data/Patient 26/s6');
...
for i in range(0,counterc+1): #this is to get maximum size of array, because it goes up to but does not include counterc unless you put +1
   if np.mean(d['C{0}'.format(i)]) > (ctotalmean - ctotalstd) and np.mean(d['C{0}'.format(i)]) < (ctotalmean + ctotalstd):
      counterctrim += 1
...
for i in range(0,counterc+1):
   if np.mean(d['C{0}'.format(i)]) > (ctotalmean - ctotalstd) and np.mean(d['C{0}'.format(i)]) < (ctotalmean + ctotalstd):
       ...
       plt.plot(t, newfile, 'g-') #adds green plot to file
...
for i in range(0,counterl+1):
   if np.mean(d['L{0}'.format(i)]) > (ltotalmean - ltotalstd) and np.mean(d['L{0}'.format(i)]) < (ltotalmean + ltotalstd):
       newfile = d['L{0}'.format(i)] * weights
       newname = 'L{0}'.format(i)
       plt.plot(t, newfile, 'b-') #adds blue plot to file
...
for i in range(0,counterr+1):
   if np.mean(d['R{0}'.format(i)]) > (rtotalmean - rtotalstd) and np.mean(d['R{0}'.format(i)]) < (rtotalmean + rtotalstd):
       newfile = d['R{0}'.format(i)] * weights
       newname = 'R{0}'.format(i) 
       plt.plot(t, newfile, 'r-') #adds red plot to the file


title1 = control[1] #takes file name from numbering of initial control name array
plt.title('S6 for Patient ' + title1[6:8]) #takes number from file name
fig = plt.gcf() #this line is required to show figure 
fig.savefig('S6 for Patient ' + title1[6:8] + '.png') #these lines are required to show figure
plt.show() #shows plot
